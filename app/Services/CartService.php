...
class CartService
{
    ...
    public function clear(): void {session()->forget([$this->key,'sarai_coupon']);}
    ...
    public function coupon(): ?Coupon { $id=session('sarai_coupon');return $id?Coupon::find($id):null; }
    public function applyCoupon(string $code): Coupon {$coupon=Coupon::whereRaw('UPPER(code)=?',[strtoupper(trim($code))])->firstOrFail();abort_unless($coupon->isValidFor($this->subtotal()),422,'This coupon cannot be applied to your cart.');session(['sarai_coupon'=>$coupon->id]);return $coupon;}
